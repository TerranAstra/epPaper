...
export default defineConfig(({ mode }) => {
        ...
        try {
          ...
          if (req.url === '/api/fs/list' && req.method === 'POST') {
            ...
            const result = await Promise.all(entries.map(async (ent) => {
              ...
              return {
                entryNameTextValue: ent.name,
                entryTypeCode: ent.isDirectory() ? 'directory' : 'file',
                entrySizeInBytesIntegerValue: stat.size,
                lastModifiedUtcEpochMsIntegerValue: stat.mtimeMs,
              };
            }));
  ...
  return {
    ...
    define: {
      'process.env.API_KEY': JSON.stringify(env.IR_BLASTER_API_KEY || ''),
    },
    resolve: {
      alias: {
