...
const LiveAudioComponent = defineComponent({
  ...
  setup(props, { emit }) {
        ...
        if (!quietAudioTimer) {
          quietAudioTimer = window.setTimeout(() => {
            if (audioReceived) {
              ...
              console.debug('[AudioAvatar] initSession(): emitting no-audio (initial quiet)');
              emit('no-audio');
            }
          }, QUIET_DURATION);
        }
    ...
    const initClient = async () => {
      console.info('[AudioAvatar] initClient(): bootstrapping audio contexts');
      initAudio();

      client = new GoogleGenAI({
        apiKey: process.env.API_KEY,
      });
      console.info('[AudioAvatar] initClient(): GoogleGenAI client ready');

      outputNode.connect(outputAudioContext.destination);
      console.info('[AudioAvatar] initClient(): output node connected');
    };

    const initSession = async () => {
      console.info('[AudioAvatar] initSession(): establishing live session', { model: selectedModel });
      audioReceived = false;
      ...
      try {
        session = await client.live.connect({
          ...
          callbacks: {
            onopen: () => {
              sessionOpen = true;
              console.info('[AudioAvatar] initSession(): live connection opened');
              updateStatus('Opened');
            },
            onmessage: async (message: LiveServerMessage) => {
              console.info('[AudioAvatar] initSession(): received server message', {
                turnComplete: message.serverContent?.turnComplete,
                hasAudio: Boolean(message.serverContent?.modelTurn?.parts?.[0]?.inlineData),
                transcription: message.serverContent?.outputTranscription?.text || null
              });
              const audio =
              ...
              if (turnComplete) {
                if (!audioReceived) {
                  ...
                  console.debug('[AudioAvatar] initSession(): emitting no-audio (turn complete)');
                  emit('no-audio');
                }
              }
            },
            onerror: (e: ErrorEvent) => {
              console.info('[AudioAvatar] initSession(): error from live connection', e.message);
              sessionOpen = false;
            ...
            onclose: (e: CloseEvent) => {
              console.info('[AudioAvatar] initSession(): connection closed', e.reason);
              sessionOpen = false;
        ...
        console.info('[AudioAvatar] initSession(): failed to start live session', e);
        sessionOpen = false;
    ...
    const startRecording = async (message: string = "hello, talk like a pirate.", voice: string, model: string) => {
      console.info('[AudioAvatar] startRecording(): invoked', { voice, model });
      if (isRecording.value) {
        console.info('[AudioAvatar] startRecording(): already recording, ignoring');
        return;
      }
      ...
      try {
        ...
        console.info('[AudioAvatar] startRecording(): session initialized');
        ...
        console.info('[AudioAvatar] startRecording(): microphone stream ready');
        ...
        scriptProcessorNode.onaudioprocess = (audioProcessingEvent) => {
          ...
          try {
            ...
            console.info('[AudioAvatar] onaudioprocess(): failed to send chunk', sendError);
            sessionOpen = false;
            if (isRecording.value) {
              stopRecording();
            }
        ...
        isRecording.value = true;
        console.info('[AudioAvatar] startRecording(): recording flag set', { isRecording: isRecording.value });
        updateStatus('🔴 Recording... Capturing PCM chunks.');

        // Only send content after session is initialized
        if (session) {
          console.info('[AudioAvatar] startRecording(): sending initial turn');
        ...
        console.info('[AudioAvatar] startRecording(): encountered error', err);
        updateStatus(`Error: ${err instanceof Error ? err.message : 'Unknown error'}`);
        stopRecording();
      }
    };
...
DO NOT JUST STAND STRAIGHT FACING THE CAMERA! DO NOT BE BORING!`;
      ...
      try {
        ...
        if (response.generatedImages && response.generatedImages.length > 0) {
          ...
          console.info('[AudioAvatar] generateImage(): received images', {
            generatedCount: response.generatedImages.length,
            usedIndex: foundNonBlack ? 'first non-black' : 'fallback-last'
          });
        ...
        console.info('[AudioAvatar] generateImage(): raw error', e);
...
Remember: You're not just speaking - you're performing poetry that moves and inspires. EVERY response MUST be in rhyming couplets with a consistent meter. Never break from the poetic form.`
    ...
    const onImFeelingLucky = () => {
      console.info('[AudioAvatar] onImFeelingLucky(): user triggered randomization');
      playClickSound();
    ...
    const onGenerateCharacter = () => {
      ...
      onStopResponse();
      console.info('[AudioAvatar] onGenerateCharacter(): starting new session');
      isConnecting.value = true;
      setTimeout(() => {
        console.info('[AudioAvatar] onGenerateCharacter(): deferred onCall');
        onCall();
    ...
    const onCall = () => {
      console.info('[AudioAvatar] onCall(): preparing prompt', {
        selectedVoice: selectedVoice.value,
        selectedDialogModel: selectedDialogModel.value,
        promptLength: characterVoiceDescription.value?.length || 0
      });
      isConnecting.value = true;
...
Current time is ${new Date().toLocaleTimeString()}. Just say a very short introduction as your character. ONLY SPEECH!!! No more than one sentence.`;
      ...
      console.info('[AudioAvatar] onCall(): dispatching startRecording');
      liveAudioRef.value?.startRecording(prompt, selectedVoice.value, selectedDialogModel.value);
      playingResponse.value = true
    };

    const handleNoAudio = () => {
      console.info('[AudioAvatar] handleNoAudio(): invoked', { noAudioCount: noAudioCount.value });
      noAudioCount.value++;
    ...
    const onStopClick = () => {
      console.info('[AudioAvatar] onStopClick(): user requested stop');
      isConnecting.value = false;
      onStopResponse()
    }

    const onStopResponse = () => {
      console.info('[AudioAvatar] onStopResponse(): stopping response');
    ...
    watch(isConnecting, (newValue, oldValue) => {
      console.info('[AudioAvatar] watch:isConnecting', {
        oldValue,
        newValue,
    ...
    const onSpeakingStart = () => {
      console.info('[AudioAvatar] onSpeakingStart(): system audio detected, clearing connecting');
      isConnecting.value = false;
...
console.info('[AudioAvatar] logging initialized', { buildTimestampIso8601: new Date().toISOString() });
const app = createApp(ImagineComponent);
app.mount('#app');
console.info('[AudioAvatar] main: mount complete');
