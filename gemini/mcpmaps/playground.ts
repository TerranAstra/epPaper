...
import {mapDataStore} from './data-store';
...
export class Playground extends LitElement {
  ...
  async renderMapQuery(location: MapParams) {
    ...
    // Extract coordinates via geocoding if we have a location query
    let coordinates: {latitude: number; longitude: number} | undefined;
    const queryToGeocode = location.location || location.search || location.origin;
    if (queryToGeocode) {
      try {
        coordinates = await this.geocodeLocation(queryToGeocode, MAPS_API_KEY);
      } catch (e) {
        console.warn('Could not geocode location:', e);
      }
    }

    // Store map interaction in BlackBox
    mapDataStore.addRecord({
      locationQuery: location.location,
      searchQuery: location.search,
      originQuery: location.origin,
      destinationQuery: location.destination,
      mapUrl: src,
      coordinates,
    });
  }

  private async geocodeLocation(query: string, apiKey: string): Promise<{latitude: number; longitude: number} | undefined> {
    try {
  ...
  exportData() {
    const jsonData = mapDataStore.exportAsJson();
    const blob = new Blob([jsonData], { type: 'application/json' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `mcpmaps-blackbox-${new Date().toISOString().split('T')[0]}.json`;
      ...
      <div class="sidebar">
        ...
        <div
          ...
          <div class="footer">
            ...
            <div id="exportArea" style="padding: 8px; border-top: 1px solid var(--color-sidebar-border); display: flex; justify-content: flex-end;">
              <button
                id="exportButton"
                @click=${() => this.exportData()}
                style="padding: 6px 12px; background: var(--color-bg2); color: var(--color-text2); border: 1px solid var(--color-sidebar-border); border-radius: 8px; cursor: pointer; font-size: 0.85em;">
            ...
            <div id="inputArea">
              <input
                ...
                @input=${(e: InputEvent) => {
                  ...
                  // Store entered text (debounced to avoid excessive records)
                  ...
                  this.inputDebounceTimer = window.setTimeout(() => {
                    if (this.inputMessage.trim().length > 0) {
                      mapDataStore.addRecord({
                        enteredText: this.inputMessage,
                      });
                    }
                  }, 500); // Store after 500ms of no typing
                }}
